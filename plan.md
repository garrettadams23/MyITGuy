...
- [ ] **Fix broken link in ntfy failure alerts** (`keep-supabase-awake.yml`): the alert message ends "…/api/db-test." — the trailing period becomes part of the tappable URL and lands on the 404 page (seen on-device 7/19). Move the URL to an ntfy `Click:` header (clean URL, whole notification tappable) and include the Actions run link (`$GITHUB_SERVER_URL/$GITHUB_REPOSITORY/actions/runs/$GITHUB_RUN_ID`).
- [ ] **Reword the keep-awake alert text**: "the site or database may be down" misled diagnosis — this check can only prove the site FUNCTION is failing. Say "the site's db-test function failed — check Netlify env/function config; the DB itself may be fine", and rely on db-health-check for real DB verdicts.
- [ ] **Land `db-health-check.yml` on `main`** (schedule/workflow_dispatch only fire from the default branch — on Testing it can never run). Then dispatch it once as a diagnostic: green ⇒ the GitHub `SUPABASE_*` secrets are valid and the prod problem is Netlify-side only; 401/403 ⇒ `SUPABASE_SECRET_KEY` itself is wrong. Requires explicit approval to push to main.
- [ ] **Root-cause the prod env value** (the actual red ✕): after the db-health verdict, add a non-secret diagnostic to `sync-secrets-to-netlify.yml` that prints each var's character length (never values) to catch empty/whitespace/malformed secrets; fix the bad value; then redeploy production (merge to main or dashboard "Trigger deploy") so functions re-bake env.
- [ ] **Auto-redeploy after env sync** (`sync-secrets-to-netlify.yml`): Netlify functions only pick up env at deploy time — this bit us three times. Add an optional `NETLIFY_BUILD_HOOK` secret and POST it at the end of a successful sync so prod redeploys automatically.
- [ ] **Pin the Supabase CLI version** in `supabase-config-push.yml` (currently `latest`; version drift caused the vector-buckets 402 failure). Also rename `[inbucket]` → `[local_smtp]` in `supabase/config.toml` (deprecation warning in run logs).
- [ ] **Bump `actions/checkout@v4` → `@v5`** in all workflows (runner logs warn v4 targets deprecated Node 20).
- [ ] **`languaage-stats.yml` cleanup**: fix the filename typo (`language-stats.yml`) and stop running a full Ruby+Linguist install on every push/PR to Testing (15+ runs this session from routine syncs alone) — switch to `workflow_dispatch` only (optionally plus a monthly cron once it lives on main).
