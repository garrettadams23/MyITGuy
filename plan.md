...
- [ ] **FAQ section + FAQPage schema**: Add an FAQ block to `index.html` answering the top questions from `marketing/content-ideas.md` (slow computer, backups on a budget, security basics), marked up with FAQPage JSON-LD so Google can surface the answers under the site's listing. Small build, high local-SEO value.
- [ ] **Publish first 3 articles**: Build a lightweight articles/tips section and publish the top three from the existing shortlist — "Why Is My Computer So Slow?" checklist, "Backup and Disaster Recovery on a Small-Business Budget", and "ServiceNow Dashboards 101". Biggest content lever, medium effort.
...
- [ ] **Text-me quick contact button**: Floating mobile "Text me" button opening SMS to 240-380-6042 prefilled — lowest-friction contact path for someone with a broken computer.
- [ ] **Lead conversion tracking**: GA4 events via the existing GTM container for contact-form submits (`/thanks.html` page view), quiz completions, and booking clicks, so lead sources become measurable. Config work in GTM/GA4, no visible site change.
- [ ] **Phone alert when the site breaks**: Reuse the ntfy setup to push an alert if the keep-supabase-awake ping fails or `submission-created.js` hits an insert error — know the site or DB is down before a customer does.
- [ ] **Performance pass**: Self-host the two Google Fonts, preload the hero background image, and defer non-critical CSS (Font Awesome, Swiper) for faster first paint on mobile and a top Lighthouse score.
