...
## Phase 15: Leads, Trust & Hygiene Pass

- [x] **Local SEO foundation**: Added `robots.txt`, `sitemap.xml`, a custom `404.html`, and a `ProfessionalService` JSON-LD block (NAP, DC service area, founder profile links) to `index.html`. Both pages got `rel=canonical`, Twitter Card tags, absolute-URL `og:image` + `og:image:alt` (social scrapers can't resolve the old relative path), and a GTM `<noscript>` fallback.
- [x] **Testimonials pipeline**: Contact form gained an opt-in "you may publish my feedback" checkbox. Migration `20260702000000_add_testimonial_publishing.sql` adds `allow_publish` + `approved` flags to `contact_submissions`; new public `netlify/functions/testimonials.js` returns name/rating/message only for rows with BOTH flags true; `index.html` has a hidden Reviews section that `script.js` populates and reveals only when approved reviews exist. Moderation = flipping `approved` in the Supabase dashboard (Table Editor → contact_submissions).
- [x] **Conversion**: Contact form now posts to a branded `/thanks.html` (noindex, GTM-tracked so form completions can become a GA4 conversion) instead of Netlify's default success screen; honeypot spam protection on both forms; second hero CTA "Book a free consult" linking to the Google Calendar embed.
- [x] **Hygiene**: Deleted 19 unreferenced media files (~16.5 MB of orphaned PNG/JPG twins of the served .webp files, an unused video, both profile-pic variants). Consolidated fonts to a single `<link>` (Poppins+Ubuntu with `display=swap` + preconnect; the old link pulled 6 families, 4 unused) and removed the render-blocking `@import` from `style.css`. Fixed a global-scope `const CONSENT_KEY` double-declaration (head inline script vs consent.js) that threw a SyntaxError and could keep the consent banner from initializing. Accessibility: real `<label for>` on all form fields, `title` on the booking iframe, keyboard access for the hamburger menu, removed an href-less anchor. `netlify.toml` force-404s repo docs/config paths (plan.md, CLAUDE.md, marketing/, supabase/, netlify/, .claude/, .github/, package*.json, .env.example) that publish="." was serving. Footer year is JS-maintained; rewrote the boilerplate `SECURITY.md`.

## Phase 16: Skills Cleanup + Phone Push Notifications
...
### Website improvement backlog (options from the Phase 15 audit — not yet built)
...
- [x] **Service landing pages**: Three dedicated pages (troubleshooting, security/software solutions, Excel & ServiceNow) targeting searches like "computer help Washington DC". Deeper SEO play, larger effort.

Features & polish:

- [x] **Text-me quick contact button**: Floating mobile "Text me" button opening SMS to 240-380-6042 prefilled — lowest-friction contact path for someone with a broken computer.
- [x] **Lead conversion tracking**: GA4 events via the existing GTM container for contact-form submits (`/thanks.html` page view), quiz completions, and booking clicks, so lead sources become measurable. Config work in GTM/GA4, no visible site change. *(Code side done — dataLayer pushes `contact_lead`, `quiz_completed`, `booking_cta_click`; the matching GTM triggers + GA4 conversions still need to be set up in the GTM UI.)*
- [x] **Phone alert when the site breaks**: Reuse the ntfy setup to push an alert if the keep-supabase-awake ping fails or `submission-created.js` hits an insert error — know the site or DB is down before a customer does.
- [ ] **Performance pass**: Self-host the two Google Fonts, preload the hero background image, and defer non-critical CSS (Font Awesome, Swiper) for faster first paint on mobile and a top Lighthouse score. *(Partly done: hero background is now preloaded with `fetchpriority="high"`. Font self-hosting and CSS deferral still pending — best done in a real Lighthouse environment, since the CDN libs don't load in the dev sandbox to verify against.)*
...
### Workflow (.github/workflows) review — July 19, 2026 (to do later)

Confirmed working: ntfy phone alerts deliver (verified on-device); the Supabase DB is ACTIVE_HEALTHY and answers direct SQL; sync-secrets and supabase-config-push run green. The keep-awake ✕ on `main` is the SITE FUNCTION failing (`CREATE_SUPABASE_CLIENT_ERROR` — a malformed `SUPABASE_*` value on the production Netlify site), NOT the database.

**Executed 2026-07-20 (on `Testing`):** items 1, 2, 5, 6, 7, 8 are done and committed. Item 3 (land db-health-check on `main`) is **deferred pending explicit approval to push to `main`**. Item 4 is partly done — the length diagnostic shipped; the remaining work (read the run log, fix the malformed prod value, redeploy prod) needs a manual sync-secrets run.

- [x] **Fix broken link in ntfy failure alerts** (`keep-supabase-awake.yml`): the alert message ends "…/api/db-test." — the trailing period becomes part of the tappable URL and lands on the 404 page (seen on-device 7/19). Move the URL to an ntfy `Click:` header (clean URL, whole notification tappable) and include the Actions run link (`$GITHUB_SERVER_URL/$GITHUB_REPOSITORY/actions/runs/$GITHUB_RUN_ID`). — *done 7/20: URL moved to `Click:` header + run link.*
- [x] **Reword the keep-awake alert text**: "the site or database may be down" misled diagnosis — this check can only prove the site FUNCTION is failing. Say "the site's db-test function failed — check Netlify env/function config; the DB itself may be fine", and rely on db-health-check for real DB verdicts. — *done 7/20: text now points at the Netlify function/env and names db-health-check for the DB verdict.*
- [ ] **Land `db-health-check.yml` on `main`** (schedule/workflow_dispatch only fire from the default branch — on Testing it can never run). Then dispatch it once as a diagnostic: green ⇒ the GitHub `SUPABASE_*` secrets are valid and the prod problem is Netlify-side only; 401/403 ⇒ `SUPABASE_SECRET_KEY` itself is wrong. Requires explicit approval to push to main. — *DEFERRED: needs your OK to push to `main`.*
- [ ] **Root-cause the prod env value** (the actual red ✕): after the db-health verdict, add a non-secret diagnostic to `sync-secrets-to-netlify.yml` that prints each var's character length (never values) to catch empty/whitespace/malformed secrets; fix the bad value; then redeploy production (merge to main or dashboard "Trigger deploy") so functions re-bake env. — *partly done 7/20: length diagnostic shipped. Remaining: run sync-secrets → production, read the lengths, fix the malformed `SUPABASE_*` secret, redeploy.*
- [x] **Auto-redeploy after env sync** (`sync-secrets-to-netlify.yml`): Netlify functions only pick up env at deploy time — this bit us three times. Add an optional `NETLIFY_BUILD_HOOK` secret and POST it at the end of a successful sync so prod redeploys automatically. — *done 7/20: optional post-sync step POSTs `NETLIFY_BUILD_HOOK_PRODUCTION` / `_TESTING` (no-op if unset).*
- [x] **Pin the Supabase CLI version** in `supabase-config-push.yml` (currently `latest`; version drift caused the vector-buckets 402 failure). Also rename `[inbucket]` → `[local_smtp]` in `supabase/config.toml` (deprecation warning in run logs). — *done 7/20: pinned `setup-cli` to `2.109.1`; config renamed to `[local_smtp]`.*
- [x] **Bump `actions/checkout@v4` → `@v5`** in all workflows (runner logs warn v4 targets deprecated Node 20). — *done 7/20: both checkout refs (supabase-config-push, language-stats) now `@v5`; the other three workflows have no checkout step.*
- [x] **`languaage-stats.yml` cleanup**: fix the filename typo (`language-stats.yml`) and stop running a full Ruby+Linguist install on every push/PR to Testing (15+ runs this session from routine syncs alone) — switch to `workflow_dispatch` only (optionally plus a monthly cron once it lives on main). — *done 7/20: renamed to `language-stats.yml`, trigger is now `workflow_dispatch` only.*
- [ ] **Minor, note only**: keep-awake's `*/6` day-of-month cron fires on the 1st right after the 31st (1-day gap at month ends) — harmless for keep-awake. Both failure alerts (keep-awake + db-health) can double-buzz during one real outage — acceptable for now.
